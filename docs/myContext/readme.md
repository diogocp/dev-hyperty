...
```
<Promise> Context.Context[] discover(HypertyResourceType[] resources)
```

**parameters**

Array of Hyperty Resources to be discovered.
...
It returns the Context Data Object created by the Syncher allowing the Application to also observe the Context with `context.onChange().then(function(){})`.
